...
class PartCrafterDiTModel(ModelMixin, ConfigMixin, PeftAdapterMixin):
    ...
    def __init__(
        ...
        enable_position_embedding=False,  # Default False for backward compatibility with old checkpoints
        ...
        enable_hierarchy_attn: bool = False,
        hierarchy_attn_block_ids: Optional[List[int]] = None,
        ...
        if enable_position_embedding:
            self.position_embedding = nn.Embedding(max_num_parts, self.inner_dim)
        ...
        # Hierarchy Attention Blocks
        self.enable_hierarchy_attn = enable_hierarchy_attn
        self.hierarchy_attn_block_ids = hierarchy_attn_block_ids or []

        if self.enable_hierarchy_attn and len(self.hierarchy_attn_block_ids) > 0:
            from src.models.hierarchy_attention import HierarchyAttentionBlock

            self.hierarchy_blocks = nn.ModuleDict({
                str(block_id): HierarchyAttentionBlock(
                    dim=self.inner_dim,
                    num_heads=self.config.num_attention_heads,
                    bidirectional=hierarchy_bidirectional,
                )
                for block_id in self.hierarchy_attn_block_ids
            })
    ...
    def forward(
        ...
        if self.enable_part_embedding:
            # Add part embedding (relative position within each object)
        ...
        if self.enable_position_embedding:
            # Add position embedding (absolute position, e.g., link_0, link_1, link_2, ...)
            # This ensures that link_0.png always corresponds to the same position embedding
            part_positions = attention_kwargs.get("part_positions", None)
            if part_positions is not None:
                if isinstance(part_positions, torch.Tensor):
                    # part_positions is already a tensor of absolute positions [N]
                    position_embedding = self.position_embedding(part_positions) # (N, D)
                elif isinstance(part_positions, list):
                    # Convert list to tensor
                    part_positions_tensor = torch.tensor(part_positions, device=hidden_states.device, dtype=torch.long)
                    position_embedding = self.position_embedding(part_positions_tensor) # (N, D)
                else:
                    raise ValueError(
                        "part_positions must be a torch.Tensor or list, but got {}".format(type(part_positions))
                    )
                hidden_states = hidden_states + position_embedding.unsqueeze(dim=1) # (N, T+1, D)
            else:
                # Fallback: if part_positions not provided, use sequential positions
                # This maintains backward compatibility
                position_embedding = self.position_embedding(torch.arange(hidden_states.shape[0], device=hidden_states.device)) # (N, D)
                hidden_states = hidden_states + position_embedding.unsqueeze(dim=1) # (N, T+1, D)

        # Clean attention_kwargs: remove part_positions as it's only used for position_embedding above
        # Keep num_parts (needed by attention processors) and parent_indices (needed by hierarchy attention)
        if attention_kwargs is not None:
            cleaned_attention_kwargs = {k: v for k, v in attention_kwargs.items() if k != 'part_positions'}
            # Use cleaned version for attention blocks
            attention_kwargs = cleaned_attention_kwargs if len(cleaned_attention_kwargs) > 0 else None
        ...
        for layer, block in enumerate(self.blocks):
            ...
            # Apply hierarchy attention if enabled for this layer
            if (self.enable_hierarchy_attn and
                layer in self.hierarchy_attn_block_ids and
                attention_kwargs is not None):

                parent_indices = attention_kwargs.get("parent_indices", None)
                num_parts = attention_kwargs.get("num_parts", None)
                if parent_indices is not None:
                    hierarchy_block = self.hierarchy_blocks[str(layer)]
                    hidden_states = hierarchy_block(
                        hidden_states=hidden_states,
                        parent_indices=parent_indices,
                        num_parts=num_parts,  # Pass num_parts for boundary validation
                    )  # (N, T+1, D)
