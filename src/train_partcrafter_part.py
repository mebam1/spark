...
def main():
    ...
    enable_position_embedding = configs["model"]["transformer"].get("enable_position_embedding", True)
    ...
    if args.from_scratch:
        ...
        transformer = PartCrafterDiTModel.from_config(
            ...
            enable_position_embedding=enable_position_embedding,
    ...
    for batch in yield_forever(train_loader):
        ...
        with accelerator.accumulate(transformer):
            ...
            # batch["images"] is expected to already contain per-part images, shape [N, H, W, 3] or List[PIL]
            ...
            # Only log on main process to avoid stdout/stderr contention in multi-GPU training
            debug_this_step = (global_update_step <= 5 or global_update_step % 100 == 0) and accelerator.is_main_process
            ...
            # HF's BitImageProcessor accepts both PIL lists and ndarrays; no manual HWC->CHW conversion needed
            with torch.no_grad():
            ...
            # ====== Part surfaces (consistent with existing pipeline) ======
            part_surfaces = batch["part_surfaces"]          # [N, P, 6]
            part_surfaces = part_surfaces.to(device=accelerator.device, dtype=weight_dtype)
            ...
            # ====== Build obj_ids: which object each part belongs to, for per-object CFG dropout ======
            ...
            if configs["train"]["cfg_dropout_prob"] > 0:
                # Decide per object whether to replace all its part image embeds with negative samples
                object_keep_mask = torch.rand(num_objects, device=accelerator.device) >= configs["train"]["cfg_dropout_prob"]  # True=keep
                part_keep_mask   = object_keep_mask[obj_ids]  # [N] mapped to each part
            ...
            # Get part_positions and parent_indices from batch
            part_positions = batch.get("part_positions", None)
            if part_positions is not None:
                part_positions = part_positions.to(device=accelerator.device)

            parent_indices = batch.get("parent_indices", None)
            if parent_indices is not None:
                parent_indices = parent_indices.to(device=accelerator.device)
            ...
            model_pred = transformer(
                ...
                attention_kwargs={
                    "num_parts": num_parts,
                    "part_positions": part_positions,  # Absolute position indices for position_embedding
            ...
            # === Longest-edge per-part weighting (intra-object normalization + clamping) ===
            # Idea: for each part, take the longest edge of its AABB (L_i); normalize relative to the
            # object-level mean: w_i = ((mean(L)_obj / (L_i + eps)) ** alpha); clamp w, then multiply
            # onto the per-part scalar loss.

            # device = loss.device
            # dtype  = loss.dtype
            # eps    = torch.tensor(1e-6, device=device, dtype=dtype)

            # # 1) Compute longest AABB edge L_i for each part
            # coords  = part_surfaces[..., :3].to(dtype)                 # [N, P, 3]
            # extents = (coords.max(dim=1).values - coords.min(dim=1).values)  # [N, 3]
            # L       = extents.max(dim=1).values + eps                  # [N] longest edge

            # # 2) Build per-part object id (reuse existing num_objects, num_parts)
            # obj_ids = torch.repeat_interleave(
            #     torch.arange(num_objects, device=device),
            #     num_parts
            # )                                                           # [N]

            # # 3) Intra-object relative normalization: divide object-level mean L by this part's L
            ...
            # alpha   = 0.7                                               # suggested tuning range: 0.5 ~ 1.0
            # w_size  = (L_mean[obj_ids] / L).pow(alpha)                  # [N] smaller parts -> larger weight

            # # 4) Clamp to avoid excessive gradient amplification (important)
            # w_min, w_max = 0.5, 2.0
            # w_size = w_size.clamp(w_min, w_max)

            # # 5) Multiply onto per-part scalar loss, then backpropagate global mean
            # loss = loss * w_size                                       # [N]
        ...
        if accelerator.sync_gradients:
            # For variable-length batches, we can't use gather() as different GPUs have different tensor sizes
            # Instead, use reduce() which averages across all processes
            # First compute mean loss on this GPU
            local_loss = loss.detach().mean()
            # Then average across all GPUs
            if accelerator.num_processes > 1:
                # Use all_reduce to average the loss across all processes
                loss_tensor = torch.tensor([local_loss.item()], device=accelerator.device)
                torch.distributed.all_reduce(loss_tensor, op=torch.distributed.ReduceOp.AVG)
                averaged_loss = loss_tensor.item()
            else:
                averaged_loss = local_loss.item()
            ...
            # Note: Skipping validation at step 1 to avoid NCCL timeout in multi-GPU training
            # Validation is slow and can cause other processes to timeout while waiting
            if args.max_val_steps > 0 and (
                (global_update_step % configs["train"]["early_eval_freq"] == 0 and global_update_step < configs["train"]["early_eval"] and global_update_step > 1)  # 1. more frequently at the beginning (skip step 1)
                ...
                # Removed: or global_update_step == 1  # Skip first step validation to prevent NCCL timeout
            ):
                ...
                # Only run validation on main process to avoid NCCL synchronization issues with variable-length batches
                if accelerator.is_main_process:
                    log_validation(
                ...
                # Wait for validation to complete on main process before continuing training
                accelerator.wait_for_everyone()
...
def log_validation(
    ...
    while val_step < args.max_val_steps:
        ...
        # Get parent_indices from batch for hierarchy attention
        parent_indices = batch.get("parent_indices", None)
        if parent_indices is not None:
            # Handle different types from dataloader
            if isinstance(parent_indices, torch.Tensor):
                # From BatchedObjaversePartDataset or default_collate
                if len(parent_indices.shape) == 2:
                    parent_indices = parent_indices[0]  # (1, N) -> (N)
                elif len(parent_indices.shape) == 1:
                    parent_indices = parent_indices  # (N,) already correct shape
                parent_indices = parent_indices.cpu().tolist()
            elif isinstance(parent_indices, list):
                # Already a list, handle nested list case
                if len(parent_indices) > 0 and isinstance(parent_indices[0], list):
                    parent_indices = parent_indices[0]  # [[...]] -> [...]
                # Otherwise keep as is
            else:
                # Unknown type, fallback
                parent_indices = [-1] * len(per_part_images)
        else:
            # Fallback: all parts are roots
            parent_indices = [-1] * len(per_part_images)
        ...
        with torch.autocast("cuda", torch.float16):
            for guidance_scale in sorted(args.val_guidance_scales):
                pred_part_meshes = pipeline(
                    ...
                    attention_kwargs={
                        "num_parts": N,
                        "part_positions": list(range(N)),  # Sequential positions for validation [0, 1, 2, ..., N-1]
                        "parent_indices": parent_indices,  # Parent index for each part (-1 for root)
                    },
                    generator=generator,
